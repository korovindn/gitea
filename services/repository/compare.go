...
func ParseCompareInfo(ctx *gitea_context.Base, baseRepo *repo_model.Repository, baseGitRepo *git.Repository, doer *user_model.User) (*CompareInfo, error) {
	...
	if rootRepo != nil &&
		...
		if canRead && rootRepo.AllowsPulls(ctx) {
...
func DownloadDiffOrPatch(ctx *gitea_context.Base, ci *CompareInfo, w io.Writer, patch, binary bool) error {
	ci.HeadGitRepo.GetDiffOrPatch(ci.BaseBranch, ci.HeadBranch, w, patch, binary)
	return nil
}
