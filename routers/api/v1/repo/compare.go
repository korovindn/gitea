...
func Compare(ctx *context.APIContext) {
	...
	diff, _, err := ci.LoadCompareDiff(ctx.Base, gitdiff.GetWhitespaceFlag(getWhitespaceBehavior(ctx)),
		nil, "")
	if err != nil {
...
func DownloadCompareDiffOrPatch(ctx *context.APIContext) {
	...
	repo_service.DownloadDiffOrPatch(ctx.Base, ci, ctx, false, false)
}
